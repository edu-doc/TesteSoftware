...
public class TesteFuncional {
     ...
     * 
     * Valida invariantes do sistema que devem ser mantidas em qualquer execução:
     * - O número total de criaturas + guardião nunca deve exceder n+1
     * - A soma total de ouro deve ser preservada no sistema
     * - O guardião sempre deve ter ID = nCriaturas + 1
     */
    @ParameterizedTest
    ...
    /**
     * Teste de Propriedade: Conservação de Energia (Ouro Total)
     * 
    ...
    /**
     * Teste End-to-End: Cenário Completo de Simulação Bem-Sucedida
     * 
     * Valida o fluxo completo desde inicialização até condição de sucesso.
     */
    @Test
     ...
     * 
     * Valida que o sistema mantém consistência em execuções consecutivas.
     */
    @Test
    void testEstresseMultiplasSimulacoes() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            resultado.forEach(iteracao -> {
                assertThat(iteracao.getGuardiao())
                        .as("Guardião deve existir em todas as iterações da simulação %d", simulacaoNumero)
                        .isNotNull();
            });
        }
    }
     ...
     * 
     * Valida que o sistema se recupera de estados potencialmente problemáticos.
     */
    @Test
    ...
    /**
     * Property-Based Test: Propriedade de conservação de criaturas
     * 
     * Testa a propriedade fundamental que o número total de elementos
     * (criaturas + clusters + guardião) permanece consistente ou diminui
     * (nunca aumenta) durante a simulação.
     */
    @ParameterizedTest
    ...
    /**
     * Property-Based Test: Propriedade de consistência da simulação bem-sucedida
     * 
     * Verifica que quando uma simulação é marcada como bem-sucedida,
     * as condições de sucesso realmente são atendidas.
     */
    @ParameterizedTest
    ...
    /**
     * Property-Based Test: Propriedade de não-negatividade do ouro
     * 
     * Verifica que o ouro de todas as entidades (criaturas, clusters, guardião)
     * nunca fica negativo durante toda a simulação.
     */
    @ParameterizedTest
        ...
        for (int i = 0; i < resultado.size(); i++) {
            ...
            for (var criatura : iteracao.getCriaturas()) {
                assertThat(criatura.getOuro())
                        .as("Iteração %d: criatura %d não deve ter ouro negativo", i, criatura.getId())
     ...
     * 
     * Executa uma simulação completa e verifica se todos os componentes
     * interagem corretamente, mantendo a integridade dos dados.
     */
    @Test
     ...
     * 
     * Verifica se a formação de clusters funciona corretamente
     * quando criaturas ocupam posições próximas.
     */
    @Test
     ...
     * 
     * Verifica se o guardião elimina clusters corretamente
     * quando ocupa posições próximas.
     */
    @Test
     ...
     * 
     * Executa múltiplas simulações idênticas e verifica que
     * os resultados são diferentes (devido à randomização).
     */
    @Test
